...
- בחירת מצלמה ספציפית (למחשבים עם כמה מצלמות).
- בדיקת מיקרופון לפני הקלטה + מד עוצמה חי.
- הקלטת ה־Workspace/Screen כווידאו `WebM` ישירות בדפדפן.
- שמירה יציבה יותר להקלטות ארוכות: כתיבה ישירה לקובץ בזמן הקלטה (כשנתמך), עם fallback אוטומטי לזיכרון.
...
- **הקלטות ארוכות יורדות פגומות / לא נפתחות**  
  ברוב המקרים שימוש בדפדפן עדכני (Chrome/Edge) פותר זאת כי מופעלת שמירה ישירה לקובץ. אם הדפדפן לא תומך בכך, האפליקציה עוברת אוטומטית ל־fallback בזיכרון.

## הערות על פורמט הקלטה

- הפורמט הנתמך לייצוא הוא `WebM` בלבד.
- עבור תאימות רחבה יותר לנגנים מסוימים, אפשר להמיר את הקובץ לאחר ההורדה ל־`MP4` בכלי חיצוני.
